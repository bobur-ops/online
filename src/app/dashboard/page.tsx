  ...
  return (
    <div className="p-2">
      ...
      <Space direction="vertical" block>
        <FancyButton onClick={() => setOpenCreateRequest(true)}>Создать заявку</FancyButton>
        <FancyButton onClick={() => setOpenCreatePreEntry(true)}>Создать Предварительную запись</FancyButton>
        <SubsidiariesTable mobile={mobile} />
      </Space>
      <CreateRequestEmployee open={openCreateRequest} onClose={() => setOpenCreateRequest(false)} mobile={mobile} />
      <CreatePreEntry open={openCreatePreEntry} onClose={() => setOpenCreatePreEntry(false)} mobile={mobile} />
    </div>
  )
}
